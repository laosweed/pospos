      ...
      <div className="flex" style={{ marginTop: 50 }}>
        ...
        <main className="flex-1 min-h-[calc(100vh-50px)] overflow-auto" style={{ marginLeft: sidebarOpen ? 200 : 0, background: "#edf1f5" }}>
          <div className="p-5 space-y-4">
            <h1 className="text-xl font-bold text-slate-800">{t("stock_hist_title")}</h1>

            <div className="grid grid-cols-3 gap-3">
              <div className="bg-white rounded-xl p-4 shadow-sm">
                <p className="text-[12px] text-slate-500 mb-1">{t("stock_hist_total_cost")}</p>
                <p className="text-xl font-bold text-blue-600">{thb(totalCost)} ฿</p>
              </div>
              <div className="bg-white rounded-xl p-4 shadow-sm">
                <p className="text-[12px] text-slate-500 mb-1">{t("stock_hist_count")}</p>
                <p className="text-xl font-bold text-emerald-600">{totalOrders} {t("stock_hist_times_unit")}</p>
              </div>
              <div className="bg-white rounded-xl p-4 shadow-sm">
                <p className="text-[12px] text-slate-500 mb-1">{t("stock_hist_avg")}</p>
            ...
            <div className="bg-white rounded-xl p-3 shadow-sm flex gap-3">
              <div className="relative flex-1">
                ...
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder={t("stock_hist_search_placeholder")}
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-100">
                      ...
                      <th className="text-left px-4 py-3 text-slate-500 font-medium">{t("stock_hist_supplier_col")}</th>
                      <th className="text-left px-4 py-3 text-slate-500 font-medium">{t("col_employee")}</th>
                      <th className="text-center px-4 py-3 text-slate-500 font-medium">{t("stock_hist_items_col")}</th>
                      <th className="text-right px-4 py-3 text-slate-500 font-medium">{t("stock_hist_total_cost_col")}</th>
                  ...
                  <tbody>
                    {filtered.map(p => (
                      <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center text-slate-600">{p.purchase_items.length} {t("stock_hist_items_col")}</td>
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={clsx("text-[11px] font-medium px-2.5 py-1 rounded-full",
                            ...
                            {p.status === "completed" ? t("stock_hist_done") : t("stock_hist_pending")}
                          </span>
                        </td>
              ...
              {!loading && filtered.length === 0 && (
                <div className="flex flex-col items-center justify-center py-16 text-slate-400 gap-2">
                  <Package size={40} strokeWidth={1} /><p>{t("stock_hist_no_data")}</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex justify-end z-50" onClick={() => setSelected(null)}>
          <div className="bg-white w-[380px] h-full overflow-auto shadow-2xl p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-bold text-lg">{t("stock_hist_detail_title")}</h2>
              <button onClick={() => setSelected(null)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
            </div>
            <div className="space-y-2 text-sm mb-4">
              <div className="flex justify-between"><span className="text-slate-500">{t("col_date")}</span><span>{fmtDate(selected.purchased_at)}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">{t("stock_hist_supplier_col")}</span><span>{selected.supplier ?? "-"}</span></div>
              ...
              {selected.note && <div className="flex justify-between"><span className="text-slate-500">{t("label_note")}</span><span>{selected.note}</span></div>}
            </div>
            ...
            <div className="border-t pt-4 flex justify-between font-bold">
              <span>{t("stock_hist_total_cost_col")}</span><span className="text-blue-600">{thb(selected.total)} ฿</span>
            </div>
          </div>
        </div>
      )}
