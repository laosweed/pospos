...
function BarcodeDisplay({ value }: { value: string }) {
  const bars = value.split("").map((c) => parseInt(c) % 2 === 0 ? 2 : 3);
  return (
    ...
    <PageShell>
      <div className="p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-slate-800">{t("barcode_title")}</h1>
          <div className="flex gap-2">
            <button className="flex items-center gap-2 border border-slate-200 bg-white text-slate-700 px-4 py-2 rounded-xl text-sm font-medium hover:bg-slate-50">
              <Download size={15} /> {t("barcode_download")}
            </button>
            <button
            ...
            >
              <Printer size={15} /> {t("barcode_print")} {selected.size > 0 ? `(${selected.size})` : ""}
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-4">
          <div className="flex-1 space-y-3">
            <div className="bg-white rounded-xl p-3 shadow-sm flex gap-3">
              ...
              <button onClick={selectAll} className="px-3 py-2 bg-slate-100 text-slate-600 rounded-lg text-sm hover:bg-slate-200">
                {selected.size === filtered.length ? t("barcode_deselect_all") : t("barcode_select_all")}
              </button>
            </div>
          ...
          <div className="w-72 flex-shrink-0 space-y-3">
            <div className="bg-white rounded-xl p-4 shadow-sm">
              <p className="text-sm font-medium text-slate-700 mb-2">{t("barcode_copies_label")}</p>
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm">
              <p className="text-sm font-medium text-slate-700 mb-3">{t("barcode_preview_label")}</p>
              {selectedProducts.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-32 text-slate-300 gap-2">
                  <span className="text-4xl">📋</span>
                  <p className="text-sm">{t("barcode_select_first")}</p>
                </div>
              ) : (
