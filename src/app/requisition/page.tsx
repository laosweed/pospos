...
export default function RequisitionPage() {
  ...
  const STATUS_STYLE = () => ({
    approved: { label: t("req_approved"),         color:"#10b981", bg:"#f0fdf4" },
    pending:  { label: t("req_pending_approval"), color:"#f59e0b", bg:"#fffbeb" },
    rejected: { label: t("req_rejected"),         color:"#ef4444", bg:"#fef2f2" },
  });
    ...
    <PageShell>
      <div className="p-5 space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-slate-800">{t("req_title")}</h1>
          <button onClick={() => setTab(tab==="list"?"new":"list")}
            ...
            {tab==="new" ? t("req_back_list") : <><Plus size={15}/>{t("req_create")}</>}
          </button>
        </div>
        ...
        {tab === "list" ? (
            ...
            {reqs.map(req => {
              ...
              return (
                <div key={req.id} className="bg-white rounded-2xl p-4 shadow-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <span className="font-semibold text-slate-800">
                          {req.type==="withdraw" ? t("req_withdraw") : t("req_return")}
                        </span>
                  ...
                  <p className="text-[12px] text-slate-500">{t("col_employee")}: {req.employee} · {t("req_reason_label")}: {req.reason}</p>
                </div>
              );
            })}
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm space-y-4 max-w-lg">
            <div className="flex gap-2">
              {(["withdraw","return"] as ReqType[]).map(tp => (
                <button key={tp} onClick={() => setType(tp)}
                  className={clsx("flex-1 py-2.5 rounded-xl text-sm font-medium transition-colors",
                    type===tp?"bg-blue-600 text-white":"bg-slate-100 text-slate-600"
                  )}>
                  {tp==="withdraw" ? t("req_withdraw") : t("req_return")}
                </button>
              ))}
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-500 mb-1.5">{t("req_reason_label")}</label>
              <input value={reason} onChange={e=>setReason(e.target.value)}
                placeholder={t("req_reason_placeholder")}
                className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:border-blue-400" />
            </div>
            <p className="text-sm text-slate-400 text-center py-4">+ {type==="withdraw" ? t("req_withdraw") : t("req_return")}</p>
