...
export default function EmployeesPage() {
  ...
  const ROLES = () => ({
    owner:   { label: t("role_owner"),   color:"#7c3aed", bg:"#f5f3ff" },
    manager: { label: t("role_manager"), color:"#1d4ed8", bg:"#eff6ff" },
    cashier: { label: t("role_cashier"), color:"#047857", bg:"#f0fdf4" },
    staff:   { label: t("role_staff"),   color:"#374151", bg:"#f9fafb" },
  });

  const load = async () => {
    const { data } = await supabase.from("employees").select("*").eq("store_id", STORE_ID).order("name");
    if (data) setEmployees(data);
    setLoading(false);
  };
  useEffect(() => { load(); }, []);

  const openNew = () => { setForm(EMPTY_FORM); setSelected({} as Employee); setIsNew(true); };
  const openEdit = (emp: Employee) => {
    setForm({ name: emp.name, email: emp.email ?? "", phone: emp.phone ?? "", role: emp.role, active: emp.active });
    setSelected(emp); setIsNew(false);
  };

  const handleSave = async () => {
    if (!form.name.trim()) return;
    setSaving(true);
    const payload = { name: form.name, email: form.email || null, phone: form.phone || null, role: form.role, active: form.active };
    if (isNew) {
        ...
        <main className="flex-1 min-h-[calc(100vh-50px)] overflow-auto" style={{ marginLeft: sidebarOpen ? 200 : 0, background: "#edf1f5" }}>
          <div className="p-5 space-y-4">
            ...
            <div className="flex items-center justify-between">
              <h1 className="text-xl font-bold text-slate-800">{t("employees_title")}</h1>
              <button onClick={openNew} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-blue-700">
                <Plus size={15} /> {t("employees_add")}
              </button>
            </div>

            <div className="grid grid-cols-3 gap-3">
              <div className="bg-white rounded-xl p-4 shadow-sm">
                <p className="text-[12px] text-slate-500 mb-1">{t("employees_total")}</p>
                <p className="text-[22px] font-bold text-blue-600">{employees.length} {t("employees_persons")}</p>
              </div>
              <div className="bg-white rounded-xl p-4 shadow-sm">
                <p className="text-[12px] text-slate-500 mb-1">{t("employees_active")}</p>
                <p className="text-[22px] font-bold text-emerald-600">{employees.filter(e=>e.active).length} {t("employees_persons")}</p>
              </div>
              <div className="bg-white rounded-xl p-4 shadow-sm">
                <p className="text-[12px] text-slate-500 mb-1">{t("employees_inactive")}</p>
                <p className="text-[22px] font-bold text-slate-400">{employees.filter(e=>!e.active).length} {t("employees_persons")}</p>
              </div>
            </div>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-2 xl:grid-cols-3 gap-4">
                {employees.map(emp => {
                  const roles = ROLES();
                  const roleInfo = roles[emp.role as keyof typeof roles] ?? roles.staff;
                  ...
                  return (
                    <div key={emp.id} className={clsx("bg-white rounded-2xl p-5 shadow-sm", !emp.active && "opacity-60")}>
                      ...
                      <div className="space-y-1.5 text-sm text-slate-500 mb-4">
                        ...
                        <p className="text-[11px]">{t("emp_joined")} {joined}</p>
                      </div>
                      <span className={clsx("text-[11px] font-medium px-2.5 py-1 rounded-full",
                        ...
                        {emp.active ? t("status_active") : t("status_inactive")}
                      </span>
                    </div>
                  );
                })}
        ...
        <div className="fixed inset-0 bg-black/40 flex justify-end z-50" onClick={() => setSelected(null)}>
          <div className="bg-white w-[380px] h-full overflow-auto shadow-2xl p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-bold text-lg text-slate-800">{isNew ? t("employees_new_title") : t("employees_edit_title")}</h2>
              <button onClick={() => setSelected(null)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
            </div>
            <div className="space-y-4">
              {([[t("label_full_name"),"name"],[t("label_email"),"email"],[t("label_phone"),"phone"]] as const).map(([l, k]) => (
                <div key={k}>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">{t("label_role")}</label>
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-slate-700">{t("label_active_status")}</span>
