...
const CATS_TH = ["ค่าวัตถุดิบ", "ค่าเช่า", "ค่าไฟ/น้ำ", "ค่าแรง", "ค่าขนส่ง", "ค่าการตลาด", "อื่นๆ"];
const INIT: Expense[] = [
...
export default function ExpensesPage() {
  ...
  const [form, setForm] = useState({ name: "", category: CATS_TH[0], amount: "", date: new Date().toISOString().split("T")[0], note: "" });
  ...
  const totalByCategory = CATS_TH.map(cat => ({
    cat,
        ...
        <main className="flex-1 min-h-[calc(100vh-50px)] overflow-auto" style={{ marginLeft: sidebarOpen ? 200 : 0, background: "#edf1f5" }}>
          <div className="p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h1 className="text-xl font-bold text-slate-800">{t("expenses_title")}</h1>
              <button onClick={() => { setForm({ name: "", category: CATS_TH[0], amount: "", date: new Date().toISOString().split("T")[0], note: "" }); setModal(true); }}
            ...
            <div className="grid grid-cols-4 gap-3">
              <div className="bg-white rounded-xl p-4 shadow-sm col-span-2">
                <p className="text-[12px] text-slate-500">{t("expenses_total")}</p>
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    <th className="text-left px-4 py-3 text-slate-500 font-medium">{t("expenses_name")}</th>
                    <th className="text-left px-4 py-3 text-slate-500 font-medium">{t("expenses_category")}</th>
                    <th className="text-left px-4 py-3 text-slate-500 font-medium">{t("expenses_date")}</th>
                    <th className="text-right px-4 py-3 text-slate-500 font-medium">{t("expenses_amount")}</th>
                ...
                <tbody>
                  {expenses.map(e => (
                    <tr key={e.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3"><span className="bg-slate-100 text-slate-600 text-[11px] px-2 py-0.5 rounded-full">{e.category}</span></td>
              ...
              {expenses.length === 0 && (
                <div className="flex flex-col items-center justify-center py-16 text-slate-400 gap-2">
                  <Wallet size={40} strokeWidth={1} /><p>{t("common_no_data")}</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setModal(false)}>
          <div className="bg-white rounded-2xl p-6 w-96 shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-bold text-lg">{t("expenses_new_title")}</h2>
              <button onClick={() => setModal(false)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="text-sm font-medium text-slate-700 block mb-1">{t("expenses_name")}</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm font-medium text-slate-700 block mb-1">{t("expenses_category")}</label>
                  <select value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))}
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-blue-400">
                    {CATS_TH.map(c => <option key={c}>{c}</option>)}
                  </select>
                </div>
                <div>
                  <label className="text-sm font-medium text-slate-700 block mb-1">{t("expenses_amount")} (฿)</label>
                  <input type="number" value={form.amount} onChange={e => setForm(p => ({ ...p, amount: e.target.value }))} min="0"
                    className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-blue-400" />
                </div>
              </div>
              <div>
                <label className="text-sm font-medium text-slate-700 block mb-1">{t("expenses_date")}</label>
            ...
            <div className="flex gap-2 mt-5">
              ...
              <button onClick={save} disabled={!form.name || !form.amount} className="flex-1 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium hover:bg-blue-700 disabled:opacity-50">{t("common_save")}</button>
            </div>
          </div>
        </div>
      )}
